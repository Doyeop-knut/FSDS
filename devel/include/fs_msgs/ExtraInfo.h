...
struct ExtraInfo_
{
  ...
  _doo_counter_type doo_counter;
   ...
   typedef std::vector<float, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<float>> _laps_type;
  _laps_type laps;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::fs_msgs::ExtraInfo_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.laps.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.laps[i]);
    }
    if (v.laps.empty() || true)
      s << "]";
  }
