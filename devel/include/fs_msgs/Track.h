...
struct Track_
{
  ...
  (void)_alloc;
   ...
   typedef std::vector< ::fs_msgs::Cone_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::fs_msgs::Cone_<ContainerAllocator> >> _track_type;
  _track_type track;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::fs_msgs::Track_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.track.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::fs_msgs::Cone_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.track[i]);
    }
    if (v.track.empty() || false)
      s << "]";
  }
